...
set(handles.hClassList, 'String', data.classes);
...
save(handles.annotfile, 'annotation')
guidata(hObject, handles);
...
dataset_globals;
im = imread(fullfile(IMAGE_DIR, sprintf('%04d.jpg', num)));
axes(handles.axes2);
imshow(im);
handles.M.image = im;
depth_file = fullfile(DEPTH_DIR, sprintf('%04d.mat', num));
if exist(depth_file, 'file')
    data = load(depth_file);
    handles.M.depth = data.depth;
else
    handles.M.depth = image;
end;
handles.xlimfull = get(handles.axes2, 'Xlim');
handles.ylimfull = get(handles.axes2, 'Ylim');
...
if ~isfield(handles, 'user')
   annotfile = fullfile(SENT_DIR, sprintf('%04d.mat', num));
    ...
    annotfile = fullfile(user_dir, sprintf('%04d.mat', num));
end;
if ~exist(annotfile, 'file')
    ...
    annotation = data.annotation;
        ...
        fprintf('problem with annotfile: %s\n', annotfile);
        data1 = load(fullfile(sprintf(SENT_USER, 'combined'), sprintf('%04d.mat', num)));
        annotation=data1.annotation;
        fprintf('no description, loaded from combined\n');
    end;
    if isempty(annotation.descriptions)
        data1 = load(fullfile(sprintf(SENT_USER, 'combined'), sprintf('%04d.mat', num)));
        annotation=data1.annotation;
        fprintf('no description, loaded from combined\n');
    end;
...
if ~isfield(annotation, 'imsize')
   annotation.imsize = size(handles.M.image);
end;
if ~isfield(annotation, 'class')
    annotation.class = [];
end;
if ~isfield(annotation, 'color')
    nobj = length(annotation.class);
    annotation.color = struct('name', cell(nobj, 1), 'brightness', cell(nobj, 1), 'difficult', cell(nobj, 1));
    for i = 1 : length(annotation.color)
        annotation.color(i).name = 'notlabeled';
        annotation.color(i).brightness = 'normal';
        annotation.color(i).difficult = 0;
    end;
    save(annotfile, 'annotation');
end;
if ~isfield(annotation, 'size')
    nobj = length(annotation.class);
    annotation.size = struct('name', cell(nobj, 1), 'difficult', cell(nobj, 1));
    for i = 1 : length(annotation.size)
        annotation.size(i).name = 'notlabeled';
        annotation.size(i).difficult = 0;
    end;
    save(annotfile, 'annotation');
end;
...
if obj_num > 0 && obj_num <= length(annotation.class)
   set(handles.hClass, 'String', annotation.class{obj_num});
else
...
annotation.color(n).name = 'notlabeled';
annotation.color(n).brightness = 'normal';
annotation.color(n).difficult = 0;
annotation.size(n).name = 'notlabeled';
annotation.size(n).difficult = 0;
handles.annotation = annotation;
handles.obj_num = n;
...
annotation = handles.annotation;
obj_num = handles.obj_num;
if obj_num == 0 || obj_num > size(annotation.bboxes, 1)
    return;
end;
set(handles.hShowMentioned, 'Value', 1);
oldbox = annotation.bboxes(obj_num, :);
padx = 0.4*oldbox(3); pady = 0.4*oldbox(4);
imsize = annotation.imsize;
if all(oldbox==0), padx = 0; pady = 0; oldbox = [padx,pady,imsize(2)-2*padx,imsize(1)-2*pady]; end;
axes(handles.axes2);
xlim([oldbox(1)-padx, oldbox(1)+ oldbox(3) + padx]);
ylim([oldbox(2)-pady, oldbox(2) + oldbox(4) + pady]);
grid on; gx = get(gca, 'XTick'); 
gy = get(gca, 'YTick'); 
set(gca, 'XTick', [10:10:600]); 
set(gca, 'YTick', [10:10:600]);
box = getrect(handles.axes2); 
handles.annotation.bboxes(obj_num, :) = box;
grid off; set(gca, 'XTick', gx); 
set(gca, 'YTick', gy);
xlim(handles.xlimfull);
ylim(handles.ylimfull);
handles.xlim = handles.xlimfull;
handles.ylim = handles.ylimfull;
annotate_objects_plots(handles);
...
polygon=impoly(handles.axes2);
points = polygon.getPosition; x = points(:, 1); y = points(:, 2); polygon.delete;
...
handles.xlim = handles.xlimfull;
handles.ylim = handles.ylimfull;
handles.annotation.seg{obj_num} = points;
bbox = [min(x), min(y), max(x) - min(x), max(y) - min(y)];
if sum(abs(handles.annotation.bboxes(obj_num, :)), 2) > 0
    pp = bbox(1:2) + bbox(3:4);
    bbox(1:2) = min(bbox(1:2), handles.annotation.bboxes(obj_num, 1:2));
    pp = max(pp, handles.annotation.bboxes(obj_num, 1:2) + handles.annotation.bboxes(obj_num, 3:4));
    bbox(3:4) = pp - bbox(1:2);
end;
handles.annotation.bboxes(obj_num, :) = bbox;
...
if ~isempty(bboxes)
   obj_num = selectObj(x, y, handles);
   if obj_num > 0
      set(handles.hObjId, 'String', sprintf('%d  /  %d', obj_num, size(bboxes, 1)));
   else
...
% --- Executes on button press in hShowAll.
function hShowAll_Callback(hObject, eventdata, handles)
% hObject    handle to hShowAll (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of hShowAll
annotate_objects_plots(handles)

% --- Executes on button press in hShowMentioned.
function hShowMentioned_Callback(hObject, eventdata, handles)
% hObject    handle to hShowMentioned (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of hShowMentioned
annotate_objects_plots(handles)

% --- Executes on button press in hShowSeg.
function hShowSeg_Callback(hObject, eventdata, handles)
% hObject    handle to hShowSeg (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of hShowSeg
annotate_objects_plots(handles)



function hClass_Callback(hObject, eventdata, handles)
% hObject    handle to hClass (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of hClass as text
%        str2double(get(hObject,'String')) returns contents of hClass as a double
annotation = handles.annotation;
obj_num = handles.obj_num;
if obj_num == 0 || obj_num > size(annotation.bboxes, 1)
    return;
end;
cls = get(handles.hClass, 'String');
handles.annotation.class{obj_num} = cls;
found = 0;
for i = 1 : length(handles.listboxtext)
   if strcmp(handles.listboxtext{i}, cls)
       found = 1;
   end;
end;
if found == 0
    handles.listboxtext{end+1} = cls;
    set(handles.hClassList, 'String', handles.listboxtext);
    classes = handles.listboxtext;
    save(handles.classfile, 'classes')
    fprintf('class names file updated\n');
end;
handles = plotstats(handles);
annotate_objects_plots(handles)
guidata(hObject, handles);
...
% --- Executes during object creation, after setting all properties.
function hClass_CreateFcn(hObject, eventdata, handles)
% hObject    handle to hClass (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in hClassList.
function hClassList_Callback(hObject, eventdata, handles)
% hObject    handle to hClassList (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns hClassList contents as cell array
%        contents{get(hObject,'Value')} returns selected item from hClassList
...
set(handles.hClass, 'String', handles.listboxtext{get(handles.hClassList, 'Value')});
handles.annotation.class{obj_num} = get(handles.hClass, 'String');
handles = plotstats(handles);
annotate_objects_plots(handles)
guidata(hObject, handles);
...
% --- Executes during object creation, after setting all properties.
function hClassList_CreateFcn(hObject, eventdata, handles)
% hObject    handle to hClassList (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in hImage.
function hImage_Callback(hObject, eventdata, handles)
% hObject    handle to hImage (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of hImage
annotate_objects_plots(handles)


% --- Executes on selection change in hColor.
function hColor_Callback(hObject, eventdata, handles)
% hObject    handle to hColor (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns hColor contents as cell array
%        contents{get(hObject,'Value')} returns selected item from hColor
if handles.obj_num > 0
   collist = get(handles.hColor, 'String');
   col = collist{get(handles.hColor, 'Value')};
   handles.annotation.color(handles.obj_num).name = col;
   brightlist = get(handles.hLight, 'String');
   bright = brightlist{get(handles.hLight, 'Value')};
   handles.annotation.color(handles.obj_num).brightness = bright;
   diff = get(handles.hColDiff, 'Value');
   handles.annotation.color(handles.obj_num).difficult = diff;
   handles = plotstats(handles);
   guidata(hObject, handles);
end;
...
% --- Executes during object creation, after setting all properties.
function hColor_CreateFcn(hObject, eventdata, handles)
% hObject    handle to hColor (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in hLight.
function hLight_Callback(hObject, eventdata, handles)
% hObject    handle to hLight (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns hLight contents as cell array
%        contents{get(hObject,'Value')} returns selected item from hLight
if handles.obj_num > 0
   collist = get(handles.hColor, 'String');
   col = collist{get(handles.hColor, 'Value')};
   handles.annotation.color(handles.obj_num).name = col;
   ...
   diff = get(handles.hColDiff, 'Value');
   handles.annotation.color(handles.obj_num).difficult = diff;
   sizelist = get(handles.hSize, 'String');
   sz = sizelist{get(handles.hSize, 'Value')};
   handles.annotation.size(handles.obj_num).name = sz;
   handles = plotstats(handles);
   guidata(hObject, handles);
end;

% --- Executes during object creation, after setting all properties.
function hLight_CreateFcn(hObject, eventdata, handles)
% hObject    handle to hLight (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in hColDiff.
function hColDiff_Callback(hObject, eventdata, handles)
% hObject    handle to hColDiff (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of hColDiff
...
function hObjNum_Callback(hObject, eventdata, handles)
% hObject    handle to hObjNum (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of hObjNum as text
%        str2double(get(hObject,'String')) returns contents of hObjNum as a double
val = get(hObject, 'String');
try
    val = str2num(val);
catch
end;
    if isnumeric(val) && val <= length(handles.annotation.class)
       handles.obj_num = val;
       handles = plotstats(handles);
       guidata(hObject, handles);
       annotate_objects_plots(handles)
...
% --- Executes during object creation, after setting all properties.
function hObjNum_CreateFcn(hObject, eventdata, handles)
% hObject    handle to hObjNum (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in hSize.
function hSize_Callback(hObject, eventdata, handles)
% hObject    handle to hSize (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns hSize contents as cell array
%        contents{get(hObject,'Value')} returns selected item from hSize

if handles.obj_num > 0
   sizelist = get(handles.hSize, 'String');
   sz = sizelist{get(handles.hSize, 'Value')};
   handles.annotation.size(handles.obj_num).name = sz;
   diff = get(handles.hSizeDiff, 'Value');
   handles.annotation.size(handles.obj_num).difficult = diff;
end;
...
% --- Executes on button press in hSizeDiff.
function hSizeDiff_Callback(hObject, eventdata, handles)
% hObject    handle to hSizeDiff (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of hSizeDiff
